...
function NotificationBell(){const{user}=useAuth();const userId=user?.id;const[count,setCount]=useState(0);useEffect(()=>{setCount(0);let active=true;if(!userId)return()=>{active=false;};const load=()=>void notificationService.getUnreadCount().then((value)=>{if(active)setCount(value);}).catch(()=>undefined);load();window.addEventListener('founder:notifications-changed',load);return()=>{active=false;window.removeEventListener('founder:notifications-changed',load);};},[userId]);return <Link to="/notifications" className="relative rounded-lg p-2 text-gray-600 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800" aria-label={`Notifications${count?` (${count} unread)`:''}`}><Bell size={20}/>{count>0&&<span className="absolute -right-1 -top-1 flex h-5 min-w-5 items-center justify-center rounded-full bg-error-500 px-1 text-[10px] font-bold text-white">{count>99?'99+':count}</span>}</Link>;}
