...
export function RealtimeCenter(){const{user,profile}=useAuth();const navigate=useNavigate();const[notice,setNotice]=useState<Notification|null>(null);const[bill,setBill]=useState<FounderBill|null>(null);const[coins,setCoins]=useState(0);
  useEffect(()=>{if(!user||!realtimeNotifications.subscribe)return;return realtimeNotifications.subscribe((incoming)=>{setNotice(incoming);window.dispatchEvent(new CustomEvent('founder:notifications-changed'));if(incoming.entityType==='bill'&&incoming.entityId&&profile?.role==='customer'){void Promise.all([founderV2Service.getBill(incoming.entityId),founderV2Service.getCoinBalance()]).then(([nextBill,balance])=>{if(nextBill){setCoins(balance);setBill(nextBill);}});}});},[profile?.role,user]);
