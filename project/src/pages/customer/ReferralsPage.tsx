...
export function ReferralsPage() {
  ...
  async function enroll() {
    setError(''); setMessage('');
    if (!VPA.test(upi.trim())) { setError('Enter a valid UPI ID, for example sahil@upi.'); return; }
    setBusy(true);
...
function Enrollment({ enrolled, upi, payee, busy, setUpi, setPayee, save, cancel }: { enrolled: boolean; upi: string; payee: string; busy: boolean; setUpi: (v: string) => void; setPayee: (v: string) => void; save: () => void; cancel: () => void }) {
  return <section className="card p-5 sm:p-7"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-brand-50 text-brand-600 dark:bg-brand-500/10"><WalletCards /></div><h2 className="mt-4 text-xl font-bold">{enrolled ? 'Edit payout UPI' : 'Start earning — free'}</h2>{!enrolled && <div className="mt-3 space-y-2 text-sm text-gray-600 dark:text-gray-300"><p>Earn <strong>₹25</strong> when a referred owner completes a verified ₹299 setup payment and their business activates.</p><p>Minimum payout is <strong>₹150</strong> — six verified businesses.</p><p className="font-semibold text-brand-600">No enrollment fee. No ₹19 payment. Referral codes are free.</p></div>}<div className="mt-5 space-y-4"><label className="block"><span className="label">UPI ID / VPA *</span><input autoCapitalize="none" className="input" placeholder="sahil@upi" value={upi} onChange={(event) => setUpi(event.target.value)} /><span className="mt-1 block text-xs text-gray-400">We never ask for your UPI PIN, OTP, or bank password.</span></label><label className="block"><span className="label">Payee name (optional)</span><input className="input" value={payee} onChange={(event) => setPayee(event.target.value)} /></label></div><div className="mt-5 flex gap-2"><button disabled={busy} onClick={save} className="btn-primary">{busy && <Loader2 size={16} className="animate-spin" />}Save &amp; Continue</button>{enrolled && <button onClick={cancel} className="btn-outline">Cancel</button>}</div></section>;
}
