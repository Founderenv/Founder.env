...
function SubscriptionManager({ businessId }: { businessId: string }) {
  ...
  return <div className="grid gap-5 lg:grid-cols-[1fr_380px]"><div className="space-y-5"><div className="card p-5"><div className="flex items-start justify-between"><div><p className="text-sm text-gray-500">Current plan</p><h2 className="mt-1 text-2xl font-bold capitalize">{sub.plan}</h2></div><StatusBadge status={sub.status} variant="success" /></div><div className="mt-5 grid gap-3 sm:grid-cols-2"><Info label="Next billing" value={sub.nextBillingDate ? formatDate(sub.nextBillingDate) : 'Not scheduled'} /><Info label="Monthly price" value="₹299/month" /></div><p className="mt-4 rounded-xl bg-gray-50 p-3 text-xs text-gray-500 dark:bg-gray-800">If Pro expires, a grace period applies before Lite. Your business profile is never deleted.</p><button onClick={() => window.alert('Billing portal integration pending.')} className="btn-outline mt-4">Manage plan</button></div><div className="card overflow-hidden"><div className="border-b p-4"><h2 className="font-semibold">Payment history & invoices</h2></div><div className="divide-y">{history.map((payment) => <div key={payment.id} className="flex items-center justify-between p-4"><div><p className="text-sm font-medium">{payment.type}</p><p className="text-xs text-gray-500">{formatDate(payment.date)} · {payment.invoiceId}</p></div><div className="text-right"><p className="text-sm font-semibold">{formatCurrency(payment.amount)}</p><button onClick={() => window.alert('Invoice download requires billing integration.')} className="text-xs text-brand-600">Invoice</button></div></div>)}</div></div></div><Summary icon={CreditCard} title="Founder.env Pro" lines={['₹599 one-time activation', '30 days Pro included', '₹299/month thereafter', 'Profile remains on Lite if unpaid']} /></div>;
}
