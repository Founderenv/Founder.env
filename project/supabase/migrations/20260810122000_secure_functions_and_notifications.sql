...
begin
  ...
  if target.id is null or target.status <> 'published' or not public.is_public_business(target.business_id)
     or now() not between target.starts_at and target.ends_at then
...
declare campaign public.scratch_campaigns; chosen jsonb; total_weight numeric; ticket numeric; result public.scratch_plays;
  reward_id uuid; reward_campaign uuid; reward_target public.reward_campaigns; reward_total bigint; eligibility_claim_id uuid;
begin
  ...
  if target_eligibility_key is null or target_eligibility_key !~* '^deal_claim:[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' then
    raise exception 'invalid eligibility key' using errcode = '22023';
  end if;
  eligibility_claim_id := split_part(target_eligibility_key, ':', 2)::uuid;
  select * into campaign from public.scratch_campaigns where id = target_campaign_id for update;
  if campaign.id is null or campaign.status <> 'published' or not public.is_public_business(campaign.business_id)
     or now() not between campaign.starts_at and campaign.ends_at then raise exception 'campaign unavailable' using errcode = 'P0002'; end if;
  if not exists(
    select 1 from public.deal_claims dc join public.deals d on d.id = dc.deal_id
    where dc.id = eligibility_claim_id and dc.customer_id = target_customer_id
      and dc.status in ('claimed','redeemed') and d.business_id = campaign.business_id
  ) then raise exception 'eligible deal claim does not belong to this campaign business' using errcode = '42501'; end if;
  ...
  with weighted as (
    select value, sum((value->>'weight')::numeric) over(order by ordinality) as ceiling
  ...
  if coalesce((chosen->>'is_win')::boolean, false) and chosen ? 'reward_campaign_id' then
    ...
    select * into reward_target from public.reward_campaigns
    where id = reward_campaign and business_id = campaign.business_id and status = 'published'
      and now() between starts_at and ends_at for update;
    if reward_target.id is null then raise exception 'reward campaign is unavailable or belongs to another business' using errcode = '23514'; end if;
    select count(*) into reward_total from public.reward_claims
    where campaign_id = reward_target.id and status in ('available','redeemed');
    if reward_target.max_claims is not null and reward_total >= reward_target.max_claims then
      raise exception 'reward campaign claim limit reached' using errcode = '23514';
    end if;
...
begin
  ...
  if target_referred_customer_id is not null and not public.is_customer(target_referred_customer_id) then
    raise exception 'referred customer is invalid' using errcode = '23514';
  end if;
  select * into campaign from public.referral_campaigns where id = target_campaign_id and status = 'published'
    and public.is_public_business(business_id) and now() between starts_at and ends_at;
  ...
  values(campaign.business_id, campaign.id, auth.uid(), target_referred_customer_id, upper(encode(extensions.gen_random_bytes(6), 'hex')))
  returning * into result;
  return result;
end;
$$;
...
begin
  ...
  values(
    target_qr.id,
    auth.uid(),
    case when target_source in ('qr','shared','direct') then target_source else 'qr' end,
    jsonb_build_object('user_agent', left(coalesce(target_metadata ->> 'user_agent', ''), 200))
  )
