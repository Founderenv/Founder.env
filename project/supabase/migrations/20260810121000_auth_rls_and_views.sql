...
returns boolean language sql stable security definer set search_path = public, pg_temp as $$
  select exists(select 1 from public.businesses b where b.id = target_business_id and b.owner_id = user_id);
$$;
...
returns boolean language sql stable security definer set search_path = public, pg_temp as $$
  select public.is_admin(user_id) or exists(
    select 1 from public.conversations c join public.businesses b on b.id = c.business_id
    where c.id = target_conversation_id and (c.customer_id = user_id or b.owner_id = user_id)
  );
$$;
...
create policy gallery_public_read on public.business_gallery for select to anon, authenticated using(exists(select 1 from public.businesses b where b.id = business_id and (b.is_active or b.owner_id = auth.uid() or public.is_admin())));
...
create policy follows_customer_insert on public.business_followers for insert to authenticated with check(customer_id = auth.uid() and public.is_customer());
...
create policy posts_read on public.posts for select to anon, authenticated using(status = 'published' or public.owns_business(business_id) or public.is_admin());
...
create policy post_media_read on public.post_media for select to anon, authenticated using(exists(select 1 from public.posts p where p.id = post_id and (p.status = 'published' or public.owns_business(p.business_id) or public.is_admin())));
...
create policy post_likes_read on public.post_likes for select to anon, authenticated using(exists(select 1 from public.posts p where p.id = post_id and p.status = 'published'));
create policy post_likes_own_insert on public.post_likes for insert to authenticated with check(customer_id = auth.uid() and public.is_customer());
create policy post_likes_own_delete on public.post_likes for delete to authenticated using(customer_id = auth.uid());
create policy comments_read on public.post_comments for select to anon, authenticated using(status = 'published' or author_id = auth.uid() or public.is_admin());
create policy comments_own_insert on public.post_comments for insert to authenticated with check(author_id = auth.uid() and exists(select 1 from public.posts p where p.id = post_id and p.status = 'published'));
...
create policy saved_posts_own on public.saved_posts for all to authenticated using(customer_id = auth.uid()) with check(customer_id = auth.uid() and public.is_customer());
create policy reposts_read on public.reposts for select to authenticated using(customer_id = auth.uid() or public.is_admin());
create policy reposts_own_write on public.reposts for insert to authenticated with check(customer_id = auth.uid() and public.is_customer());
create policy reposts_own_delete on public.reposts for delete to authenticated using(customer_id = auth.uid());

create policy stories_read on public.stories for select to anon, authenticated using((status = 'published' and expires_at > now()) or public.owns_business(business_id) or public.is_admin());
...
create policy highlights_read on public.story_highlights for select to anon, authenticated using(exists(select 1 from public.businesses b where b.id = business_id and (b.is_active or b.owner_id = auth.uid() or public.is_admin())));
...
create policy highlight_items_owner_write on public.story_highlight_items for all to authenticated using(exists(select 1 from public.story_highlights h where h.id = highlight_id and (public.owns_business(h.business_id) or public.is_admin()))) with check(exists(select 1 from public.story_highlights h where h.id = highlight_id and (public.owns_business(h.business_id) or public.is_admin())));

create policy deals_read on public.deals for select to anon, authenticated using(status = 'published' or public.owns_business(business_id) or public.is_admin());
...
create policy saved_deals_own on public.saved_deals for all to authenticated using(customer_id = auth.uid()) with check(customer_id = auth.uid() and public.is_customer());

create policy clips_read on public.deal_clips for select to anon, authenticated using(status = 'published' or public.owns_business(business_id) or public.is_admin());
...
create policy clip_likes_read on public.deal_clip_likes for select to anon, authenticated using(exists(select 1 from public.deal_clips c where c.id = clip_id and c.status = 'published'));
create policy clip_likes_own_insert on public.deal_clip_likes for insert to authenticated with check(customer_id = auth.uid() and public.is_customer());
create policy clip_likes_own_delete on public.deal_clip_likes for delete to authenticated using(customer_id = auth.uid());
create policy saved_clips_own on public.saved_deal_clips for all to authenticated using(customer_id = auth.uid()) with check(customer_id = auth.uid() and public.is_customer());

create policy reviews_public_read on public.reviews for select to anon, authenticated using(status = 'approved' or customer_id = auth.uid() or public.is_admin());
create policy reviews_customer_insert on public.reviews for insert to authenticated with check(customer_id = auth.uid() and public.is_customer());
...
create policy replies_public_read on public.review_replies for select to anon, authenticated using(exists(select 1 from public.reviews r where r.id = review_id and r.status = 'approved'));
...
create policy helpful_read on public.review_helpful for select to anon, authenticated using(true);
create policy helpful_own on public.review_helpful for all to authenticated using(customer_id = auth.uid()) with check(customer_id = auth.uid());
...
create policy conversations_customer_insert on public.conversations for insert to authenticated with check(customer_id = auth.uid() and public.is_customer());
create policy conversations_owner_insert on public.conversations for insert to authenticated with check(public.owns_business(business_id));
...
create policy messages_participant_insert on public.messages for insert to authenticated with check(sender_user_id = auth.uid() and public.can_access_conversation(conversation_id));
...
create policy reward_campaigns_read on public.reward_campaigns for select to anon, authenticated using(status = 'published' or public.owns_business(business_id) or public.is_admin());
...
create policy scratch_campaigns_read on public.scratch_campaigns for select to authenticated using(status = 'published' or public.owns_business(business_id) or public.is_admin());
...
create policy referral_campaigns_read on public.referral_campaigns for select to authenticated using(status = 'published' or public.owns_business(business_id) or public.is_admin());
...
create policy loyalty_programs_read on public.loyalty_programs for select to authenticated using(status = 'published' or public.owns_business(business_id) or public.is_admin());
...
create policy loyalty_members_customer_insert on public.loyalty_members for insert to authenticated with check(customer_id = auth.uid());
...
revoke insert, update, delete on public.payments, public.invoices from anon, authenticated;
